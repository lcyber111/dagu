...
现有多租户系统 `occ2-setup` 由 FastAPI 管理服务（`occ-user-manager`）和 Caddy 网关组成：门户通过 `POST/DELETE /admin/api/users/{uid}` 同步创建/删除用户容器，Caddy 负责 `/u/{uid}` 路由与 Cookie 注入。
...
1. 保留 Caddy 作为网关容器：沿用 occ2-setup 的 `caddy-gateway/Caddyfile` 为基础修改，继续负责 `/u/{uid}` 用户流量路由（写 `ws_user` Cookie、转发到 `dagu-u-{uid}:4096`、`/app-proxy/<port>` 通用转发），并把 `/api/v1/webhooks/*` 转发到 dagu 的 webhook 端点。
...
- 用户访问入口保留（`http://IP:9088/u/{uid}`），与旧系统体验一致。
